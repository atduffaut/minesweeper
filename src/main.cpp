...
int main()
{
    ...
    if (!inFile.is_open()) cout << "Couldn't open file" << endl;
    ...
    while (window.isOpen()) {
        ...
        while (mineCount != mines && !randomized) {
            int rand = Random::Number(0, columns * rows - 1);
            if (!tiles[rand].mine) {
        ...
        for (int i = 0; i < columns; i++) {
            for (int j = 0; j < rows; j++) {
                ...
                if ((tiles[rows * i + j].mine && debugOn) || tiles[rows*i + j].mine && mineOn) {
                ...
                if (!win && !lose) {
                    face_happy.setPosition(columns * 16 - 32, rows * 32);
                    window.draw(face_happy);
                }
                if (win) {
                    face_win.setPosition(columns * 16 - 32, rows * 32);
                    window.draw(face_win);
                }
                if (lose) {
                    face_lose.setPosition(columns * 16 - 32, rows * 32);
                    window.draw(face_lose);
                }
                debug.setPosition(columns * 16 + 96, rows * 32);
                window.draw(debug);
                test_1.setPosition(columns * 16 + 160, rows * 32);
                window.draw(test_1);
                test_2.setPosition(columns * 16 + 224, rows * 32);
                window.draw(test_2);
                test_3.setPosition(columns * 16 + 288, rows * 32);
                window.draw(test_3);
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                ...
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    if (event.mouseButton.x > columns * 16 + 160 && event.mouseButton.x < columns * 16 + 224 && event.mouseButton.y > rows * 32 && event.mouseButton.y < rows * 32 + 64) {
                    ...
                    if (event.mouseButton.x > columns * 16 + 224 && event.mouseButton.x < columns * 16 + 288 && event.mouseButton.y > rows * 32 && event.mouseButton.y < rows * 32 + 64) {
                        randomized = true;
                        clickable = true;
                        win = false;
                        lose = false;
                    ...
                    if (event.mouseButton.x > columns * 16 + 288 && event.mouseButton.x < columns * 16 + 352 && event.mouseButton.y > rows * 32 && event.mouseButton.y < rows * 32 + 64) {
                        randomized = true;
                        clickable = true;
                        win = false;
                        lose = false;
                    ...
                    if (clickable && event.mouseButton.x > columns * 16 + 96 && event.mouseButton.x < columns * 16 + 160 && event.mouseButton.y > rows * 32 && event.mouseButton.y < rows * 32 + 64) {
                        if (!debugOn) {
                            debugOn = true;
                        }
                        else if (debugOn) {
                    ...
                    if (event.mouseButton.x > columns * 16 - 32 && event.mouseButton.x < columns * 16 + 32 && event.mouseButton.y > rows * 32 && event.mouseButton.y < rows * 32 + 64) {
                        randomized = false;
                        clickable = true;
                        win = false;
                        lose = false;
